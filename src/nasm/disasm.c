	     ...
	     int autosync, unsigned long prefer)
{
	    ...
	    (ins.oprs[i].segment & SEG_RMREG)) 
	{
	    ...
	    name = syms_val2name(ins.oprs[i].offset, &delta);
	    if(name && !((*p)->opd[i] & BITS8))
	    {
		if(delta>0)
	            slen += sprintf(output+slen, "0x%lx", ins.oprs[i].offset);
		else
		    slen += sprintf(output+slen, "%s", name);
	    } else {
	        slen += sprintf(output+slen, "0x%lx", ins.oprs[i].offset);
	    }
	} else if ( !(MEM_OFFS & ~(*p)->opd[i]) ) {
	    slen += sprintf(output+slen, "[%s%s%s",
			    (segover ? segover : ""),
			    (segover ? ":" : ""),
	    ...
	    {
		if(delta>0)
	            slen += sprintf(output+slen, "%s+0x%lx]", name, delta);
		else
		    slen += sprintf(output+slen, "%s]", name);
	    } else {
	        slen += sprintf(output+slen, "0x%lx]", ins.oprs[i].offset);
		...
		if(name)
		{
			...
			slen += sprintf(output+slen, "0x%lx",
					ins.oprs[i].offset);
		}
	    }
	    output[slen++] = ']';
